...
impl Config {
    pub fn load() -> Self {
        // Load .env file if present (useful in development)
        if let Err(e) = dotenvy::dotenv() {
            info!("No .env file found or error loading it: {}. Relying on environment variables.", e);
        }
        ...
        let session_timeout_minutes: u64 = env::var("SESSION_TIMEOUT_MINUTES")
            ...
            .parse()
            .expect("SESSION_TIMEOUT_MINUTES must be a valid u64");

        let heartbeat_timeout_seconds: u64 = env::var("HEARTBEAT_TIMEOUT_SECONDS")
            .unwrap_or_else(|_| "60".to_string())
            .parse()
            .expect("HEARTBEAT_TIMEOUT_SECONDS must be a valid u64");

        let max_message_size: usize = env::var("MAX_MESSAGE_SIZE")
            .unwrap_or_else(|_| "65536".to_string())
            .parse()
            .expect("MAX_MESSAGE_SIZE must be a valid usize");

        let max_connections: usize = env::var("MAX_CONNECTIONS")
            .unwrap_or_else(|_| "1000".to_string())
            .parse()
            .expect("MAX_CONNECTIONS must be a valid usize");
        ...
        let turn_url = env::var("TURN_URL").ok().filter(|s| !s.trim().is_empty());
        let turn_username = env::var("TURN_USERNAME").ok().filter(|s| !s.trim().is_empty());
        let turn_password = env::var("TURN_PASSWORD").ok().filter(|s| !s.trim().is_empty());
